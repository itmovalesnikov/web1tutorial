...
URL, который мы вводим в браузер выглядит, как 
`<протокол>://<адрес>/<путь>`. В нашем случае путь URL соответствует 
...
принято оставлять исполняемые файлы без расширений *(в отличие от `.exe`
на Windows)*.
...
*(root - корень, так называют папку относительно которой ищутся
...
его при запуске
  ...
  можно указать его при запуске, но придётся каждый раз писать лишние буквы)*
...
мне перегруженным, сложным для понимания и изменения под конкретного 
...
Для начала нужно подключиться к helios и клонировать этот репозиторий
```bash
...
pkill httpd
```
В репозитории лежат более аккуратные скрипты запуска,
...
поверх ssh, через защищённый канал. Работает это так, что
...
свойство среды выполнения java.
- `-jar web1.jar` - запускает ваш jar файл, можно назвать его и по-другому
